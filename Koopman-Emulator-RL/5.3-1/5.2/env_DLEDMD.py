...
class env_DLEDMD:
    def __init__(self,date_time,date_t):
        ...
        with g_DLEDMD.as_default():
            ...
            print(np.min(r1))
            self.sess.run(self.op,feed_dict={self.DL.x:X_train,self.DL.f:X_train_F,self.DL.y:Y_train})
            saver.save(self.sess,'./emulator_model/DLEDMD/DLEDMD_model.ckpt')
            '''
            saver.restore(self.sess,'./emulator_model/DLEDMD_emulator_model/DLEDMD_model.ckpt')
    ...
    def reset(self,raindata):
        ...
        st=np.array([0,0,0,0]).reshape((1,-1))

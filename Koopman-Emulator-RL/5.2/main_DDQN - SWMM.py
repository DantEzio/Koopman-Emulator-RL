...
if __name__=='__main__':
    ...
    g1=tf.Graph()
    with g1.as_default():
        D_DQN = DDQN.DDQN(step=step,batch_size=batch_size,num_rain=rain_num,env=envD,t='dqn_SWMM',raindata='test')
        t1=time.time()
        D_DQN.train(True)
        t2=time.time()
        print(t2-t1)
    ...
    with g2.as_default():
        ...
        D_DQN = DDQN.DDQN(step=step,batch_size=batch_size,num_rain=rain_num,env=envS,t='dqn_SWMM',raindata='real')
        D_DQN.load_model()
        D_DQN.test(test_num)

# -*- coding: utf-8 -*-
"""
Created on Mon Dec 20 13:30:07 2021
...
class env_MLP:
    def __init__(self, date_time, date_t):
        ...
        #Linear
        self.reg = MLPRegressor(hidden_layer_sizes=(10,50,100,100,50,10,4),  activation='relu', alpha=0.1, batch_size='auto',
                  power_t=0.5, max_iter=5000,epsilon=1e-05,solver='adam')
        '''
        epoch=1
        for step in np.arange(epoch):
            self.reg.fit(X_train,Y_train)
        
        s=pickle.dumps(self.reg)
        with open('./dqn_MLP_test_result/emulator model/MLP.model','wb+') as f:
            f.write(s)
        '''
        
        f=open('./dqn_MLP_test_result/emulator model/MLP.model','rb')
        s=f.read()
        self.reg=pickle.loads(s)
